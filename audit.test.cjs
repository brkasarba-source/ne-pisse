...
test('All 238 legacy rows are accounted for; no duplicate active IDs or names', () => {
  ...
  const mains = meals.filter(m => !m.companionOnly);
  assert.equal(mains.filter(m => m.status === 'sourced').length,143);
  assert.equal(mains.filter(m => m.status === 'idea').length,92);
  assert.equal(meals.filter(m => m.companionOnly).length,7);
  // companion-only recipes must never reach the wheel
  for (const m of meals.filter(m => m.companionOnly)) {
    assert.equal(matchesMeal(m, {maxTime:Infinity}),false);
    assert.equal(matchesMeal(m, {includeCompanions:true, maxTime:Infinity}),true);
  }
  assert(!meal(190));
...
test('Selected companions list separately, scale, preserve checks, and reset with a new main', () => {
  const s=setup();
  ...
  // Companion ingredients get their own block; amounts are never merged across recipes.
  const groups=s.eval('shoppingGroups()');
  assert.equal(groups.length,2);
  assert.equal(groups[0].recipe.id,'meal-19');
  assert.equal(groups[1].recipe.id,'meal-23');
  const headings=Array.from(s.elements.ingredients.children).filter(li=>!li.shoppingKey);
  assert.equal(headings.length,2);
  assert(headings[0].textContent.startsWith('Ana tarif · '));
  assert(headings[1].textContent.startsWith('Eşlikçi · '));
  // Same-name ingredient in two recipes stays on two separate lines.
  const butterRows=s.eval('shoppingGroups().flatMap(g => g.rows).filter(r => typeof r.item === "object" && r.item.name === "tereyağı" && r.item.unit === "yemek kaşığı")');
  assert(butterRows.length >= 1);
  const before=s.eval('shoppingGroups()[0].rows.find(r => typeof r.item === "object" && r.item.name === "tereyağı" && r.item.unit === "yemek kaşığı").item.amount');
  assert.equal(before, 4 * 2 / meal(19).yieldPeople);
  const firstRow=Array.from(s.elements.ingredients.children).find(li=>li.shoppingKey);
  firstRow.children[0].children[0].checked=true;
  s.elements.plus.click();
  const sameRow=Array.from(s.elements.ingredients.children).find(li=>li.shoppingKey);
  assert(sameRow.children[0].children[0].checked);
  assert.equal(s.eval('shoppingGroups()[0].rows.find(r => typeof r.item === "object" && r.item.name === "tereyağı" && r.item.unit === "yemek kaşığı").item.amount'),before*1.5);
  select.value=''; select.onchange(); assert.equal(s.eval('selectedSides.size'),0);
  assert.equal(Array.from(s.elements.ingredients.children).filter(li=>li.shoppingKey).length,meal(19).ingredients.length);
  select.value='meal-23'; select.onchange();
  s.eval('showMeal(byId.get("meal-1"))'); assert.equal(s.eval('selectedSides.size'),0);
});
test('Batch-limited scaling warning appears only when it applies', () => {
  const s=setup();
  // Lahmacun is oven/tray bound; a sote scales linearly.
  const yieldPeople=meal(10).yieldPeople;
  s.eval('showMeal(byId.get("meal-10"))');
  s.eval('people='+yieldPeople); s.eval('renderMetrics()');
  assert(!s.elements.portionNote.textContent.includes('tek tava/tepsi'));
  s.eval('people='+(yieldPeople+2)); s.eval('renderMetrics()');
  assert(s.elements.portionNote.textContent.includes('tek tava/tepsi'));
  // A recipe without the flag never shows it, however many people.
  const linearId=s.eval('meals.find(m => m.status==="sourced" && !m.batchLimited && m.yieldPeople===2).id');
  s.eval('showMeal(byId.get("'+linearId+'"))');
  s.eval('people=8'); s.eval('renderMetrics()');
  assert(!s.elements.portionNote.textContent.includes('tek tava/tepsi'));
  // The caveat is no longer baked into note text for anyone.
  for (const m of s.eval('meals')) {
    assert(!(m.note||'').includes('daha büyük miktarlar için aynı süre garanti edilmez'));
  }
});
test('Spoon conversions stay on the three-teaspoon tablespoon scale', () => {
  const s=setup();
  // This app treats 1 yemek kaşığı = 3 çay kaşığı = 15 ml, so çay kaşığı is a 5 ml
  // teaspoon and 1/4 çay kaşığı is a real measuring-spoon step, not an artefact.
  assert.equal(s.eval('practicalAmount(1/3, "yemek kaşığı", "tuz")'),'1 çay kaşığı tuz');
  assert.equal(s.eval('practicalAmount(1, "tatlı kaşığı", "tuz")'),'1 tatlı kaşığı tuz');
  assert.equal(s.eval('practicalAmount(0.25, "çay kaşığı", "karabiber")'),'1/4 çay kaşığı karabiber');
  assert.equal(s.eval('practicalAmount(0.5, "çay kaşığı", "tuz")'),'1/2 çay kaşığı tuz');
  // Nothing is ever rendered below the smallest spoon in a standard set.
  for (const value of [0.01,0.05,0.1,0.2]) {
    assert.equal(s.eval(`practicalAmount(${value}, "çay kaşığı", "x")`),'yaklaşık 1/4 çay kaşığı x');
  }
});
